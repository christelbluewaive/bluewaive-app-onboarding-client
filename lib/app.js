...
// Table "Paramètres" : fiche unique contenant les coordonnées réelles de Bluewaive
// elle-même (déjà utilisée en dehors de ce portail par les workflows n8n de
// génération de contrat - lue ici en LECTURE SEULE, jamais modifiée). Voir
// fetchAdminSettings ci-dessous : seule source fiable pour Téléphone/Adresse de
// la fiche Admin, la fiche BLUEWAIVE de la table Agences a ces deux champs vides.
const AIRTABLE_TABLE_PARAMETRES = process.env.AIRTABLE_TABLE_PARAMETRES || 'Paramètres';
...
async function findAgencyByEmail(email) {
  ...
  // Echappement formule Airtable : antislash AVANT guillemet (sinon un antislash deja
  // present dans l'email pourrait se combiner avec celui insere ci-dessous pour refermer
  // la chaine plus tot que prevu et casser la structure de la formule).
...
// ---- Console Admin (etape 1) : liste des clients actuels Bluewaive ----
// Critere "client actuel" retenu (audit dedie, voir conversation) : acces portail
// accorde (`Accès App Onboarding` = true) ET pas le compte Admin lui-meme. Choix
// deliberement INDEPENDANT de Retell/Voice OS - un client sans agent vocal encore
// configure doit quand meme apparaitre ici, contrairement a
// fetchAdminTestableAgencies ci-dessus (dediee au seul selecteur du widget WebCall,
// qui lui a besoin d'un Retell Agent ID valide). Un client sans acces portail
// (prospect en cours de discussion commerciale) n'apparait pas ici.
async function fetchAdminClientAgencies() {
  if (!AIRTABLE_API_KEY) return [];
  const records = await fetchAirtableList(AIRTABLE_BASE_ID, AIRTABLE_API_KEY, AIRTABLE_TABLE_AGENCES, 'TRUE()');
  const clientRecords = records
    .map((record) => ({ id: record.id, fields: record.fields || {} }))
    .filter((entry) => {
      const accesOnboarding = Boolean(extractValue(entry.fields, ['Accès App Onboarding'], false));
      return accesOnboarding && normalizePortalRole(entry.fields) !== 'admin';
    });

  // Volume tres faible (quelques agences) - boucle sequentielle simple, meme
  // tolerance deja acceptee ailleurs (fetchAirtableRecords) pour ce volume.
  const clients = [];
  for (const entry of clientRecords) {
    const fields = entry.fields;
    // Reutilise normalizeOffer telle quelle (deja utilisee par buildLiveData) -
    // jamais de logique de lecture d'offre dupliquee.
    const offre = await normalizeOffer(extractValue(fields, ['Offre Souscrite', 'offreSouscrite'], null));
    // Reutilise buildProjectSteps telle quelle (deja utilisee par buildLiveData) -
    // simple comptage d'etapes completees, aucune nouvelle logique de progression.
    const projectSteps = buildProjectSteps(
      Boolean(extractValue(fields, ['Audit fait', 'auditFait'], false)),
      Boolean(extractValue(fields, ['Config Retell faite', 'configRetellFaite'], false)),
      Boolean(extractValue(fields, ['Formation faite', 'formationFaite'], false)),
      Boolean(extractValue(fields, ['Mise en production', 'miseEnProduction'], false)),
      Boolean(extractValue(fields, ['Suivi actif', 'suiviActif'], false))
    );
    clients.push({
      id: entry.id,
      nomAgence: extractValue(fields, ['Nom Agence', 'NomAgence', 'name'], ''),
      contact: extractValue(fields, ['Contact Référent', 'Prénom', 'Prenom'], ''),
      email: extractValue(fields, ['Email', 'email'], ''),
      // 3 champs Statut distincts et reellement presents sur les fiches (voir audit) -
      // exposes separement plutot que d'en deviner un seul, la Console Admin choisira
      // lequel afficher.
      statut: extractValue(fields, ['Statut'], ''),
      statutCommercial: extractValue(fields, ['Statut Commercial', 'statutCommercial'], ''),
      statutProjet: extractValue(fields, ['Statut Projet'], ''),
      accesOnboarding: Boolean(extractValue(fields, ['Accès App Onboarding'], false)),
      offre: offre.nom,
      agentVocal: extractValue(fields, ['Agent Vocal', 'agentVocal'], ''),
      // Uniquement un booleen expose au frontend, jamais l'identifiant technique
      // voice_os_agency_id brut (meme precaution que le correctif deja applique a
      // buildLiveData pour le payload client).
      voiceOsActif: Boolean(extractValue(fields, ['voice_os_agency_id'], '')),
      progression: {
        completed: projectSteps.filter((step) => step.completed).length,
...
// Coordonnees reelles de Bluewaive (Profil Admin, "Votre compte" pour l'admin) -
// lues depuis la fiche unique de la table "Paramètres" (voir constante ci-dessus).
// Cette fiche contient aussi des champs sensibles (IBAN, BIC, Lien Stripe, SIRET,
// Email Pro OVH, Delai Paiement) : SEULS Telephone et Adresse sont extraits ici,
// jamais l'objet complet, jamais transmis au frontend au-dela de ces deux champs.
async function fetchAdminSettings() {
  if (!AIRTABLE_API_KEY) return null;
  try {
    const records = await fetchAirtableList(AIRTABLE_BASE_ID, AIRTABLE_API_KEY, AIRTABLE_TABLE_PARAMETRES, 'TRUE()');
    const fields = (records[0] && records[0].fields) || {};
    return {
      telephone: extractValue(fields, ['Téléphone', 'Telephone'], ''),
      adresse: extractValue(fields, ['Adresse'], '')
    };
  } catch (error) {
    console.warn(`Paramètres (Profil Admin) error: ${error.message}`);
    return null;
  }
}
...
// ---- Console Admin (etape 2) : agence effectivement consultee ----
// Erreur typee (jamais renvoyee telle quelle au navigateur, uniquement un message
// neutre) : permet au routeur de distinguer "cible invalide" d'une vraie panne
// Airtable (503) sans dupliquer la logique de reponse a chaque appelant.
class InvalidTargetAgencyError extends Error {}

// Determine l'agence dont les donnees doivent etre chargees, sans jamais modifier
// session.agencyId (identite du compte connecte, immuable pour toute la requete).
// Meme patron que create-web-call (lib/app.js, route create-web-call) : le
// `targetAgencyId` recu du navigateur n'est qu'un pointeur d'enregistrement, jamais
// une donnee de confiance en soi - revalide integralement cote serveur via un fetch
// Airtable frais avant tout usage.
//
// - Client normal (role !== 'admin') : targetAgencyId totalement ignore, quelle que
//   soit sa valeur - le contexte reste strictement session.agencyId. Un client ne
//   peut donc jamais obtenir les donnees d'une autre agence par ce mecanisme.
// - Admin sans targetAgencyId : reste sur son propre contexte (session.agencyId,
//   la fiche BLUEWAIVE) - jamais de selection automatique/aleatoire d'un client.
// - Admin avec targetAgencyId : la fiche cible doit exister ET remplir exactement
//   le meme critere "client actuel" que fetchAdminClientAgencies ci-dessus (acces
//   portail accorde + role different d'Admin) - exclut donc naturellement BLUEWAIVE
//   elle-meme comme cible, et tout identifiant invalide/inexistant/non-client.
async function resolveEffectiveAgencyId(session, targetAgencyId) {
  if (session.role !== 'admin') {
    return session.agencyId;
  }
  const trimmedTarget = String(targetAgencyId || '').trim();
  if (!trimmedTarget) {
    return session.agencyId;
  }
  let targetRecord;
  try {
    targetRecord = await fetchAirtableRecord(AIRTABLE_TABLE_AGENCES, trimmedTarget);
  } catch (fetchError) {
    targetRecord = null;
  }
  if (!targetRecord) {
    throw new InvalidTargetAgencyError('agence cible introuvable');
  }
  const targetFields = targetRecord.fields || {};
  const accesOnboarding = Boolean(extractValue(targetFields, ['Accès App Onboarding'], false));
  const isClientRole = normalizePortalRole(targetFields) !== 'admin';
  if (!accesOnboarding || !isClientRole) {
    throw new InvalidTargetAgencyError('agence cible non consultable (pas un client actuel)');
  }
...
async function buildLiveData(agencyId) {
  ...
  try {
    ...
    // Profil Admin : Telephone/Adresse completes depuis la table Paramètres (voir
    // fetchAdminSettings) - uniquement pour la fiche Admin elle-meme (jamais pour
    // un client), et uniquement si la fiche Agences n'a pas deja ces valeurs (ce
    // qui est le cas actuel pour BLUEWAIVE, verifie). Ecrase seulement les champs
    // vides, ne remplace jamais une vraie valeur deja presente sur la fiche.
    if (normalizePortalRole(fields) === 'admin') {
      const adminSettings = await fetchAdminSettings();
      if (adminSettings) {
        if (!compte.telephone && adminSettings.telephone) compte.telephone = adminSettings.telephone;
        if (!compte.adresse && adminSettings.adresse) compte.adresse = adminSettings.adresse;
      }
    }
    ...
    // voiceOsAgencyId a deja servi ci-dessus (fetchVoiceOsStats) - identifiant technique
    // interne, jamais utile au frontend (qui ne consomme que voiceOsStats deja calcule) :
    // exclu de l'objet envoye au navigateur, sans toucher a son usage serveur.
    const { voiceOsAgencyId, ...agencyForClient } = agency;

    return {
      agency: agencyForClient,
      devis,
      factures,
      contrat,
      cahierDesCharges,
...
function resolvePagePath(requestPath) {
  ...
  // Les pages client (client-home.html, client-devis.html, etc.) ne sont PLUS
  // resolues ici : elles restent exclusivement accessibles via les routes protegees
  // /client/:agencyId/* (voir pageFiles plus bas, sous garde de session). Audit
  // securite etape 6B : ces 10 entrees dupliquaient pageFiles sans aucune garde de
  // session, inutilisees par la navigation/le code applicatif (verifie exhaustivement).
...
async function handleRequest(req, res) {
  ...
  if (clientMatch) {
    ...
    // Console Admin (etape 1) : liste des clients actuels Bluewaive. Reservee au
    // role admin (403 sinon, meme regle que les routes admin ci-dessus/ci-dessous) -
    // GET uniquement, aucune ecriture. `role` vient exclusivement de la session deja
    // verifiee (jamais d'une valeur fournie par le navigateur).
    if (isApiSubrequest && req.method === 'GET' && pathname === `/client/${agencyId}/api/admin-clients`) {
      if (session.role !== 'admin') {
        sendJson(res, 403, { error: "Action réservée à l'équipe Bluewaive." });
        return;
      }
      try {
        const clients = await fetchAdminClientAgencies();
        sendJson(res, 200, { clients });
      } catch (error) {
        console.warn(`admin-clients error: ${error.message}`);
        sendJson(res, 503, { error: 'Impossible de récupérer la liste des clients pour le moment.' });
      }
      return;
    }
    ...
    if (isApiSubrequest && req.method === 'GET') {
      const apiPath = pathname.replace(`/client/${agencyId}/api/`, '');
      // Console Admin (etape 2) : ?targetAgencyId=... optionnel, uniquement pris en
      // compte pour un compte admin (resolveEffectiveAgencyId l'ignore totalement
      // pour un client normal - session.agencyId, deja verifie ci-dessus, reste
      // alors la seule source possible). Jamais un remplacement de session.agencyId
      // lui-meme, uniquement de l'ID transmis a buildLiveData ci-dessous.
      // "compte" (page "Votre compte"/"Profil Admin") = identite du compte CONNECTE,
      // jamais celle de l'agence consultee - targetAgencyId n'est donc jamais pris
      // en compte pour cet endpoint precis, meme pour un admin (correctif : avant ce
      // correctif, un admin ayant selectionne une agence dans le bandeau voyait les
      // coordonnees de CETTE agence apparaitre sur sa propre page Profil Admin).
      // Les 8 autres endpoints de la liste ci-dessous continuent de respecter
      // targetAgencyId normalement (inchange, etape 2).
      const targetAgencyIdParam = apiPath === 'compte' ? null : reqUrl.searchParams.get('targetAgencyId');
      let effectiveAgencyId;
      try {
        effectiveAgencyId = await resolveEffectiveAgencyId(session, targetAgencyIdParam);
      } catch (error) {
        console.warn(`resolveEffectiveAgencyId: ${error.message}`);
        sendJson(res, 404, { error: 'Agence cible introuvable ou non accessible.' });
        return;
      }
      let payload;
      try {
        payload = await buildLiveData(effectiveAgencyId);
      } catch (error) {
    ...
    // Page "Clients" (Console Admin, etape 4A) : reservee au role admin. Un client
    // normal recoit le meme 404 neutre que toute route qui ne le concerne pas -
    // jamais de fuite d'existence de cette page (meme principe que les routes API
    // admin deja gardees par un role, applique ici a une page HTML plutot qu'a du
    // JSON). `session.role` vient exclusivement de la session deja verifiee.
    if (subpage === 'clients' && session.role !== 'admin') {
