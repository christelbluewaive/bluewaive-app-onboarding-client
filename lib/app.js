...
async function fetchVoiceOsStats(voiceOsAgencyId) {
  ...
  try {
    ...
    for (const record of leadRecords) {
      ...
      if (leadFields["Date de l'appel"]) {
        activity.push({ type: 'lead', label: `Lead créé - ${prenomNom}`, date: leadFields["Date de l'appel"] });
      }
      ...
      if (leadFields['Date du rendez-vous']) {
        rdvCount += 1;
        activity.push({ type: 'rdv', label: `RDV réservé - ${prenomNom}`, date: leadFields['Date du rendez-vous'] });
      }
    }
    ...
    for (const record of relanceRecords) {
      ...
      activity.push({ type: 'relance', label: `Relance créée - ${relanceFields['Prospect'] || ''}`, date: relanceDate });
    }
...
async function buildLiveData(agencyId) {
  try {
    ...
    const voiceOsStats = agency.voiceOsAgencyId ? await fetchVoiceOsStats(agency.voiceOsAgencyId) : null;
