...
function injectAssetVersion(html) {
  return html
    ...
    .replace(/(src="\/scripts\/(?:client-portal|auth|donut)\.js)(")/g, `$1?v=${ASSET_VERSION}$2`);
}
...
async function handleRequest(req, res) {
  ...
  if (clientMatch) {
    ...
    // agencyId). L'agent_id n'est JAMAIS fourni par le navigateur ni par le corps de la
    // requete - toujours resolu cote serveur depuis le champ Airtable "Retell Agent ID"
    // de la fiche agence authentifiee. La cle RETELL_API_KEY reste strictement cote
    // serveur (postJsonToUrl/createRetellWebCall) - jamais transmise au frontend.
    if (isApiSubrequest && req.method === 'POST' && pathname === `/client/${agencyId}/api/create-web-call`) {
      try {
        if (isWebCallRateLimited(agencyId)) {
          sendJson(res, 429, { error: 'Veuillez patienter quelques secondes avant de relancer un appel test.' });
          return;
        }
        const agencyRecord = await fetchAirtableRecord(AIRTABLE_TABLE_AGENCES, agencyId);
        const fields = agencyRecord ? (agencyRecord.fields || {}) : {};
        ...
        if (!retellAgentId || !RETELL_AGENT_ID_PATTERN.test(retellAgentId)) {
          ...
          console.warn(`create-web-call: Retell Agent ID manquant ou invalide pour l'agence ${agencyId}`);
        ...
        // variable) si le champ Airtable correspondant est vide.
        ...
        console.log(`create-web-call: session créée pour l'agence ${agencyId}${callId ? ` (call_id ${callId})` : ''}`);
        ...
        console.warn(`create-web-call error for agency ${agencyId}: ${error.message}`);
