...
async function buildLiveData(agencyId) {
  try {
    ...
    return {
      ...
      contrat,
      projectSteps,
      // MVP à un seul client (démo/fictif) : tant qu'aucune vraie clé RETELL_API_KEY
      // n'est branchée, on affiche des chiffres de démo plutôt que des zéros.
      // À retirer dès qu'un vrai client est onboardé avec son propre numéro Retell.
      retellStats: retellStats || {
        callCount: 24,
        averageDurationMinutes: 3.6,
        status: 'Actif',
        lastCalls: [
          { id: 'demo-call-1', datetime: '2026-08-04 10:15', durationMinutes: 3.8, status: 'Terminé' },
          { id: 'demo-call-2', datetime: '2026-08-04 09:20', durationMinutes: 5.1, status: 'Terminé' },
          { id: 'demo-call-3', datetime: '2026-08-03 18:05', durationMinutes: 2.4, status: 'Terminé' }
        ]
      }
    };
  } catch (error) {
    console.warn(`Falling back to mock data because Airtable/Retell integration failed: ${error.message}`);
...
async function handleRequest(req, res) {
  ...
  if (clientMatch) {
    ...
    if (isApiSubrequest) {
      ...
      if (['devis', 'factures', 'contrat', 'retell-stats', 'overview'].includes(apiPath)) {
