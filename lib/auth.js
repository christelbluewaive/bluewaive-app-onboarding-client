...
// `role` ('admin' | 'client') est fourni par l'appelant (lib/app.js, lu depuis
// le champ Airtable "Rôle Portail" au moment du login) - jamais devine ici.
// Signe dans le jeton au meme titre que agencyId/email : la seule maniere de
// changer le role d'une session deja emise est de se reconnecter.
function signSession({ agencyId, email, role }) {
  return signPayload({ agencyId, email, role, purpose: 'session' }, SESSION_TTL_SECONDS);
}
